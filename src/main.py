...
async def _ensure_legacy_unversioned_schema() -> None:
    ...
    async with engine.begin() as conn:
        ...
        await conn.execute(text("ALTER TABLE reminders ADD COLUMN IF NOT EXISTS driver_document_id INTEGER"))
        ...
        await conn.execute(text("CREATE INDEX IF NOT EXISTS ix_reminders_driver_document_id ON reminders (driver_document_id)"))
        await conn.execute(text("CREATE INDEX IF NOT EXISTS ix_reminders_driver_document_status ON reminders (driver_document_id, status)"))
        ...
        await conn.execute(text("ALTER TABLE medication_intakes ADD COLUMN IF NOT EXISTS scheduled_slot_at_utc TIMESTAMPTZ"))
        await conn.execute(text("ALTER TABLE medication_intakes ADD COLUMN IF NOT EXISTS medication_name_snapshot VARCHAR(255)"))
        await conn.execute(text("ALTER TABLE medication_intakes ADD COLUMN IF NOT EXISTS dosage_snapshot VARCHAR(255)"))
        await conn.execute(text("ALTER TABLE medication_intakes ADD COLUMN IF NOT EXISTS instructions_snapshot TEXT"))
        await conn.execute(text("ALTER TABLE medication_intakes ADD COLUMN IF NOT EXISTS importance_snapshot VARCHAR(20)"))
        await conn.execute(text("CREATE INDEX IF NOT EXISTS ix_medication_intakes_scheduled_slot_at_utc ON medication_intakes (scheduled_slot_at_utc)"))
        await conn.execute(text("CREATE INDEX IF NOT EXISTS ix_medication_intakes_med_slot ON medication_intakes (medication_id, scheduled_slot_at_utc)"))
...
async def run_bot() -> None:
    ...
    heartbeat = ServiceHeartbeatWriter(
        ...
        metadata_factory=lambda: {
            "handlers_count": count_bot_handlers(bot_app),
            "webhook_configured": bool(settings.WEBHOOK_URL),
            "polling_started": polling_started,
        },
    )
    heartbeat_task = asyncio.create_task(heartbeat.run(), name="heartbeat-bot")
